...
#include "Yolov8ssegNppModel.hpp"
...
using Yolov8ssegNppClassifier = arm::app::Classifier;
...
namespace arm {
    namespace app {
        ...
        bool ClassifyImageHandler(ApplicationContext& ctx)
        {
            ...
            const uint32_t nCols = inputShape->data[arm::app::Yolov8ssegNppModel::ms_inputColsIdx]; // W
            const uint32_t nRows = inputShape->data[arm::app::Yolov8ssegNppModel::ms_inputRowsIdx]; // H
            const uint32_t nChannels = inputShape->data[arm::app::Yolov8ssegNppModel::ms_inputChannelsIdx];   // C
            info("input shape: %" PRIu32 ", %" PRIu32 ", %" PRIu32 ", %d\n", nCols, nRows, nChannels, inputShape->data[0]);
            info("yolov8ssegnpp signed %d.\n", model.IsDataSigned());
            ...
            /* Set up post-processing. */ // 没有考虑降低分辨率的情况output_num_boxes
            ImgClassPostProcess postProcess =
                ImgClassPostProcess(outputTensors,
                    ctx.Get<Yolov8ssegNppClassifier&>("classifier"),
            ...
            /* Scratch buffer in DDR for per-detection mask computation (no heap alloc) */
            static uint8_t combined_mask[640 * 640]
                __attribute__((section("activation_buf_dram"), aligned(16)));
            postProcess.SetScratchBuffer(combined_mask, nCols, nRows);


            bool useCamera = true;

#if defined(BUILD_FVP_TESTS) || defined(USE_SINGLE_INPUT)
            useCamera = false;
            info("Using static input data for FVP/test mode\n");
#endif
            ...
            while (true) {
                ...
                uint32_t capturedFrameSize = 0;     // size of captured frame
                static uint32_t frame_idx = 0;      // index of current frame
                const uint8_t* imgSrc = nullptr;    // pointer to image data

                if (useCamera) {
                    hal_lcd_clear(COLOR_BLACK);
                    hal_camera_start();
                    /* get input data from image.cc */
                    imgSrc = hal_camera_get_captured_frame(&capturedFrameSize);
                    if (!imgSrc || !capturedFrameSize) {
                        break;
                    }
                } else {
                    imgSrc = get_sample_data_ptr(frame_idx);
                    capturedFrameSize = get_sample_img_total_bytes();
                    if (!imgSrc || !capturedFrameSize) {
                        break;
                    }
                }
                ...
                /* Get image ID(image index which is used for map json) from filename. */
                int img_id = 0;
                ...
                /* Get image ratio and padding form pre-processing. */
