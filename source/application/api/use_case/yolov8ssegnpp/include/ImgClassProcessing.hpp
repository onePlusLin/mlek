...
namespace arm {
    namespace app {
        ...
        class ImgClassPostProcess : public BasePostProcess {
            ...
            /**
             * @brief       Set scratch buffer for per-detection mask computation.
             * @param[in]   buf    Pre-allocated buffer (model_w × model_h bytes), in DDR.
             * @param[in]   w      Buffer width in pixels.
             * @param[in]   h      Buffer height in pixels.
             **/
            void SetScratchBuffer(uint8_t* buf, int w, int h);
            ...
            uint8_t* m_scratchBuf = nullptr;  // Scratch buffer for per-detection mask (DDR)
            int m_scratchW = 0;
            int m_scratchH = 0;
        };
